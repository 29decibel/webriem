...
Menu.create(:name=>'fees',:title=>'费用类型',:path=>'/model_search/index?class_name=Fee',:menu_type=>0)
Menu.create(:name=>'regions',:title=>'地区',:path=>'/model_search/index?class_name=Region',:menu_type=>0)
Menu.create(:name=>'transportations',:title=>'交通方式',:path=>'/model_search/index?class_name=Transportation',:menu_type=>0)
Menu.create(:name=>'lodgings',:title=>'住宿方式',:path=>'/model_search/index?class_name=Lodging',:menu_type=>0)
Menu.create(:name=>'settlements',:title=>'结算方式',:path=>'/model_search/index?class_name=Settlement',:menu_type=>0)
Menu.create(:name=>'fee_standards',:title=>'费用标准',:path=>'/model_search/index?class_name=FeeStandard',:menu_type=>0)
Menu.create(:name=>'projects',:title=>'项目',:path=>'/model_search/index?class_name=Project',:menu_type=>0)
Menu.create(:name=>'budgets',:title=>'预算数据',:path=>'/model_search/index?class_name=Budget',:menu_type=>0)
Menu.create(:name=>'currencies',:title=>'币种',:path=>'/model_search/index?class_name=Currency',:menu_type=>0)
Menu.create(:name=>'subjects',:title=>'科目设置',:path=>'/model_search/index?class_name=Subject',:menu_type=>0)
Menu.create(:name=>'accounts',:title=>'账户设置',:path=>'/model_search/index?class_name=Account',:menu_type=>0)
Menu.create(:name=>'work_flows',:title=>'审批流设置',:path=>'/model_search/index?class_name=WorkFlow',:menu_type=>0)
#docs menu goes here
