...
class JjPdf 
  ...
  def self.to_pdf(pdf,doc)
    ...
    if doc.rd_work_meals.count>0
      ...
      pdf.move_down 2
      pdf.table doc.rd_work_meals.map {|r| ["#{r.dep ? r.dep.name : ""}","#{r.project ? r.project : ""}","#{r.place}","#{r.meal_date}","#{r.people_count}","#{r.person_names}","#{r.currency.name}","#{r.rate}","#{r.ori_amount}","#{r.apply_amount}"]},
