...
class JjPdf < Prawn::Document
  ...
  def to_pdf
    ...
    if doc.rd_work_meals.count>0
      ...
      text "交际费明细",:size=>12
      move_down 2
      table doc.rd_work_meals.map {|r| ["#{r.dep ? r.dep.name : ""}","#{r.project ? r.project.name : ""}","#{r.place}","#{r.meal_date}","#{r.people_count}","#{r.person_names}","#{r.currency.name}","#{r.rate}","#{r.ori_amount}","#{r.apply_amount}"]},
