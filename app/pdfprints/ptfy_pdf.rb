...
class PtfyPdf < Prawn::Document
  ...
  def to_pdf
    ...
    if doc.reim_cp_offsets.count>0
      ...
      move_down 2
      table doc.reim_cp_offsets.where("amount>0").all.map {|r| ["#{r.cp_doc_head.doc_no}","#{r.cp_doc_head.apply_date}","#{r.cp_doc_head.project}","#{r.cp_doc_head.total_apply_amount}","#{r.amount}"]},
