...
class ClPdf 
  def self.to_pdf(pdf,doc)
    ...
    #title
     pdf.text "差旅费报销单",:size=>18,:align=>:center
    #image
    ...
    if doc.rd_transports.count>0
      pdf.move_down 10
       pdf.text "交通费明细",:size=>12
      pdf.move_down 2
      pdf.table doc.rd_transports.map {|r| ["#{r.start_date}","#{r.end_date}","#{r.start_position}","#{r.end_position}","#{r.ori_amount}","#{r.rate}","#{r.apply_amount}","#{r.hr_amount}","#{r.fi_amount}"]},
    ...
    if doc.rd_lodgings.count>0
      pdf.move_down 10
       pdf.text "住宿费明细",:size=>12
      pdf.move_down 2
      pdf.table doc.rd_lodgings.map {|r| ["#{r.start_date}","#{r.end_date}","#{r.days}","#{r.region}","#{r.people_count}","#{r.ori_amount}","#{r.rate}","#{r.apply_amount}","#{r.hr_amount}","#{r.fi_amount}"]},
    ...
    if doc.other_riems.count>0
      pdf.move_down 10
       pdf.text "其他费用明细",:size=>12
      pdf.move_down 2
      pdf.table doc.other_riems.map {|r| ["#{r.sequence}","#{r.description}","#{r.ori_amount}","#{r.rate}","#{r.apply_amount}","#{r.hr_amount}","#{r.fi_amount}"]},
    ...
    if doc.recivers.count>0
      ...
      pdf.move_down 2
      data_array=doc.recivers.map {|r| ["#{r.settlement}","#{r.company_name}","#{r.bank}","#{r.bank_no}","#{r.amount}","#{r.hr_amount}","#{r.fi_amount}"]}
