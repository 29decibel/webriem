...
class JbfPdf < Prawn::Document
  ...
  def to_pdf
    ...
    table [
      ...
      ["费用承担部门","#{doc.dep.name}","是否分摊","#{doc.is_split==1 ? "是" : "否"}"]],
    ...
    if doc.rd_extra_work_meals.count>0
      ...
      text "加班餐费明细",:size=>12
      move_down 2
      table doc.rd_extra_work_meals.map {|r| ["#{r.is_sunday==1 ? "是" : "否" }","#{r.start_time}","#{r.end_time}","#{r.currency.name}","#{r.ori_amount}","#{r.rate}","#{r.apply_amount}","#{r.hr_amount}","#{r.fi_amount}"]},
    ...
    if doc.rd_extra_work_cars.count>0
      ...
      text "加班车费明细",:size=>12
      move_down 2
      table doc.rd_extra_work_cars.map {|r| ["#{r.is_sunday==1 ? "是" : "否" }","#{r.start_place}","#{r.end_place}","#{r.start_time}","#{r.end_time}","#{r.currency.name}","#{r.ori_amount}","#{r.rate}","#{r.apply_amount}","#{r.hr_amount}","#{r.fi_amount}"]},
